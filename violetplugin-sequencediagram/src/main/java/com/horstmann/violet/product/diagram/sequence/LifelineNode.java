 ...
 */
public class LifelineNode extends RectangularNode//生命线节点
{
    /**
    ...
    {
        if (!n.getClass().isAssignableFrom(ActivationBarNode.class))//如果n不是activationBarNode类型
        {
            return false;
        }//则返回false
        n.setParent(this);//将生命线NODE设为activationBar节点的父节点
        n.setGraph(getGraph());
        n.setLocation(p);//将n节点的位置根据p点设置
     ...
     */
    private INode getNearestNodeAfterThisPoint(Point2D p)//通过点p来获取最近的节点
    {
        double y = p.getY();

 ...
 */
public class ClassNode extends RectangularNode implements IResizableNode
{
    /**
    ...
    public ClassNode()
    {
        ...
        name.setSize(MultiLineString.LARGE);//设置name属性
        attributes = new MultiLineString();//设置attributes属性
        attributes.setJustification(MultiLineString.LEFT);//设置attributes的位置
        methods = new MultiLineString();//设置methods方法属性
        methods.setJustification(MultiLineString.LEFT);//设置methods方法的位置
        
    }
    
    private Rectangle2D getTopRectangleBounds() {//获取顶部矩形的边界
        Rectangle2D globalBounds = new Rectangle2D.Double(0, 0, 0, 0);
        Rectangle2D nameBounds = name.getBounds();
        globalBounds.add(nameBounds);
        boolean isMethodsEmpty = (methods.getText().length() == 0);//判断方法是不是为空
        boolean isAttributesEmpty = (attributes.getText().length() == 0);//判断属性是不是为空

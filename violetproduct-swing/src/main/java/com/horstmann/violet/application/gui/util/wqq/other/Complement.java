...
public class Complement {
	...
	public static ArrayList<DBM_element[][]> complement(DBM_element[][] DBM){
		...
		DBM_element[][] fDBM=Floyds.floyds(DBM);//必须将DBM先规范化，再求补集
		ArrayList<DBM_element[][]> complements=new ArrayList<DBM_element[][]>();
	
		for(int i=0;i<dimension;i++){
			for(int j=0;j<dimension;j++){
				
				if(i==0&&fDBM[i][j].getValue()!=0){//处理第一行元素
					DBM_element com_DBM[][]=new DBM_element [dimension][dimension];
					DBM_element d=new DBM_element();
					d.setDBM_i(j);
					d.setDBM_j(i);
					d.setStrictness(!fDBM[i][j].isStrictness());
					d.setValue(-fDBM[i][j].getValue());
					com_DBM[j][i]=d;	
					
					complements.add(com_DBM);
				}
				
				if(i>0&&i!=j&&fDBM[i][j].getValue()!=88888){//处理剩下的元素(对角线上元素不处理)
					DBM_element com_DBM[][]=new DBM_element [dimension][dimension];
					DBM_element d=new DBM_element();
					d.setDBM_i(j);
					d.setDBM_j(i);

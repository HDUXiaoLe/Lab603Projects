...
//    
    ...
    public void write(IGraph graph, OutputStream out)
    {
        try
        {
            OutputStreamWriter writer = new OutputStreamWriter(out, "UTF-8");//将输出流写成UTF-8格式
        	//InputStream templateAsStream = this.getClass().getResourceAsStream(TEMPLATE_FILE);
            //String template = getInputStreamContent(templateAsStream);//将TEMPLATE_FILE写成UTF-8格式
    ...
    public IGraph read(InputStream in) throws IOException
    {
       ...
       // HTMLEditorKit.Parser parser = kit.getParser();
        ...
        String xmlContent =  getInputStreamContent(in);    

...
public class ManiocFramework {
        ...
        public static class BeanInjector {
              ...
              //建立一个哈希MAP映射 将ClassLoader和BeanInjector联系起来
                /**
                 * Singleton constructor
                 */
                private BeanInjector() {//定义一个私有的构造函数
                        // Singleton pattern
                }
              //单例模式
                /**
                ...
                public static BeanInjector getInjector() {
                        ...
                        //获取BeanInjector该类的类加载器（加载类的对象）
                        if (!instances.containsKey(currentClassLoader)) {//如果该MAP集合没有currentClassLoader这个键值
                                BeanInjector newInstance = new BeanInjector();//则新建一个BeanInjecrtor对象
                                instances.put(currentClassLoader, newInstance);//然后设置键值对
                        }
                        return instances.get(currentClassLoader);//返回当前类加载器对应的BeanInjector对象
                }
                ...
                public void inject(Object o, Class<? extends DefaultApplicationContext> context) {
                        List<Class<?>> classAndSuperClasses = getClassAndSuperClasses(o);//返回这个对象所在的类和超类
                                ...
                                for (Field aField : aClass.getDeclaredFields()) {
                                	...
                                	//  返回 Field 对象的一个数组，这些对象反映此 Class对象所表示的类或接口所声明的所有字段。
                                        ...
                                        if (propertyAnnotation != null) {
                                                aField.setAccessible(true);//私有变量要设置此条语句
                                                if (!isFieldNUll(aField, o)) {//如果有字段直接进行下一次循环
                                                        continue;
                                                }
                                                Class<?> beanType = aField.getType();
                                                boolean isInterface = beanType.isInterface();
                                                if (isInterface) {//如果该声明字段对应的类是一个接口类型
                                                        boolean isImplementationFound = false;//设置实现方法标志为False
                                                        // Step 1 : take default implementation//采取默认实现
                                                        //注解用于指定一个接口的默认实现
                                                        ...
                                                        // directly on the field//用另一个类重写他？
                                                        ...
                                                        //工厂会自动创建bean实例基于注释(\ @ManagedBean)。如果无法自动创建bean,别担心。你可以手动在工厂中注册。

...
public class XStreamBasedPersistenceService implements IFilePersistenceService {
	...
	public IGraph read(InputStream in) throws IOException {
		...
		XStream xStream = new XStream(new DomDriver("UTF-8"));//XStream是一个xml序列化工具
		//用“UTF-8”编码的方式编写xml文件
		xStream = getConfiguredXStream(xStream);
		//xStream.alias("horizontalchild",HorizontalChild.class);//在反序列化的时候也需要将别名改成对应类名
		Object fromXML = xStream.fromXML(reader);//反序列化reader.也就是输入的数据
		IGraph graph = (IGraph) fromXML;//把输入的数据还原成一个graph
	...
	private XStream getConfiguredXStream(XStream xStream) {
		...
		xStream.setMode(XStream.ID_REFERENCES);//id的引用

...
public class ScrollPaneNavigatorPanel extends JPanel
{
    ...
    public ScrollPaneNavigatorPanel(JScrollPane aScrollPane)
    {
        ...
        theStartPoint = null;
        theScale = 0.0;
        setCursor(Cursor.getPredefinedCursor(Cursor.MOVE_CURSOR));//鼠标光标类型
        MouseInputListener mil = new MouseInputAdapter()
        {
            public void mousePressed(MouseEvent e)
            {
                theStartPoint = e.getPoint();//起点是鼠标点击的点
            }

            public void mouseReleased(MouseEvent e)
            {
                if (theStartPoint != null)//如果鼠标一开始有点击
                {
                    Point newPoint = e.getPoint();//newPoint 是鼠标释放的点
                    int deltaX = (int) ((newPoint.x - theStartPoint.x) / theScale);
                    //deltaX是水平方向的距离
                    int deltaY = (int) ((newPoint.y - theStartPoint.y) / theScale);
                    //deltaY是垂直方向的距离
                    scroll(deltaX, deltaY);//置滚动矩形可视化
                }
                theStartPoint = null;
...
//         thePopupMenu.setBorder(BorderFactory.createLineBorder(Color.BLACK));
    ...
    protected void paintComponent(Graphics g1D)
    {
        ...
        int xOffset = insets.left;//左边的inset
        int yOffset = insets.top;//上面的inset

...
public class MultiLineString implements Serializable, Cloneable {
	...
	public void draw(Graphics2D g2, Rectangle2D r) {
		getLabel().setBounds((int)r.getX(),(int)r.getY(), (int) r.getWidth(), (int) r.getHeight());
		g2.translate(r.getX(), r.getY());
		getLabel().paint(g2);
		g2.translate(-r.getX(), -r.getY());
	}
	public void drawTimingDiagram(Graphics2D g2,Rectangle2D r)
	{
		getLabel().setBounds((int)r.getX(),(int)r.getY(), (int) r.getWidth(), (int) r.getHeight());
		g2.translate(r.getX(), r.getY());
		getLabel().paintTimingDiagarm(g2);
		g2.translate(-r.getX(), -r.getY());
	}
   // public 
	...
	public MultiLineLabel getLabel() {//¸ü¸ÄÎªPUBLIC
		if (this.label == null) {

...
public abstract class RectangularNode extends AbstractNode implements IColorable
{
    ...
    private List<IEdge> getEdgesOnSameSide(IEdge edge) {
        ...
        List<IEdge> result = new ArrayList<IEdge>();//定义一个Result集合。包含所有同侧边
        Direction d = edge.getDirection(this);//d为边的方向

        if (d == null) return result;
        Direction cardinalDirectionToSearch = d.getNearestCardinalDirection();
        //cardinalDirectionToSearch=边的大概朝向（东，南，西，北）
        for (IEdge anEdge : getConnectedEdges()) {//迭代
            Direction edgeDirection = anEdge.getDirection(this);//edgeDirection=边相对于节点的方向
            Direction nearestCardinalDirection = edgeDirection.getNearestCardinalDirection();
            //nearestCardinalDirection=边对于节点的大概朝向（东南西北）
        ...
        }//如果两条边相连在同一侧，且是南北方向，也就是上下方向，则比较他们的横向分量

...
public abstract class EllipticalNode extends RectangularNode
{
    ...
    public Point2D getConnectionPoint(IEdge e)
    {
        ...
        double a = bounds.getWidth() / 2;//a是外围矩形的宽度的1/2
        double b = bounds.getHeight() / 2;//b是外围矩形的高度的1/2
        double x = d.getX();//得到这个方向上的X分量
        double y = d.getY();//得到这个方向上的Y分量
        double cx = bounds.getCenterX();//获取外围矩形的中心X坐标
        double cy = bounds.getCenterY();//获取外面矩形的中心Y坐标
